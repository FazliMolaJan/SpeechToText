package com.fahmtechnologies.speechtotext.Network;

public class WebFields {

    public static String BASE_REQ_URL = "https://translate.yandex.net/api/v1.5/tr.json/";
    public static final String API_KEY = "trnsl.1.1.20170425T085917Z.580fec9ed721d387.9caf5bd2514ca1581b95de321974dce826ef5064";
    public static final String APP_OPEN_LOG = "https://vps.fahm-technologies.com/api/apps/stt/appopenlog";


    public static final String REQUEST_LATITUDE  = "latitude";
    public static final String REQUEST_LONGITUDE  = "longitude";
    public static final String REQUEST_LATLOGADDRESS  = "latlogaddress";
    public static final String REQUEST_ANDROID_VERSION  = "androidversion";
    public static final String REQUEST_DEVICE_NAME  = "devicename";
    public static final String REQUEST_IP_ADDRESS  = "ipaddress";
    public static final String REQUEST_MOBILEDETAILS  = "mobiledetails";
    public static final String REQUEST_UUID  = "uuid";
    public static final String RESPONSE_STATUS = "status";


}
